...
namespace DB_Backend.Controllers
{
    ...
    public class TicketsController : Controller
    {
        ...
        public IActionResult GetPassenger([FromBody] PassengerGetModle model)
        {
            ...
            Console.WriteLine(response[0].IsStudent);
            return Ok(response);
        }
        ...
        public IActionResult ReduceTickets([FromBody] TicketsReduceOrAddModel model)
        {
            TicketsResponseModel response = TicketsManager.ReduceOrAddTickets(model);
            return Ok(response);
        }
        ...
        /// <summary>
        /// 支付订单接口
        /// 调用方法：post <address>/api/tickets/payOrder
        /// </summary>
        /// <param name="model"></param>
        /// <returns></returns>
        [HttpPost("payOrder")]
        public IActionResult PayOrder([FromBody] PayOrderModel model)
        {
            if (string.IsNullOrEmpty(model.OrderID)) {
                Console.WriteLine("empty");
            }
            Console.WriteLine(model.OrderID);
            TicketsResponseModel response = TicketsManager.PayOrder(model);
            return Ok(response);
        }
