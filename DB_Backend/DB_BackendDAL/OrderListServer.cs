...
namespace DB_Backend.DB_BackendDAL
{
    public class OrderListServer
    {
        ...
        public static DataTable TotalMoney(int tickettype)
        {
            ...
            string sql = $@"SELECT SUM(PRICE) AS TotalPrice FROM ORDERLIST WHERE TICKET_TYPE ='{ticketTypeCondition}' AND ORDER_STATUS = 'paid' ";
            DataTable dt = DBServer.FetchData(sql);
            return dt;
        }

        public static DataTable TotalWait(int tickettype)
        {
            string ticketTypeCondition;
            switch (tickettype)
            {
            ...
            string sql = $@"SELECT COUNT(*) FROM ORDERLIST WHERE TICKET_TYPE = '{ticketTypeCondition}' AND ORDER_STATUS = 'wait'";
            DataTable dt = DBServer.FetchData(sql);
            return dt;
        }
    }
